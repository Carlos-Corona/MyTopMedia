...
function showForm(data) {
  ...
  main.innerHTML = '';
  console.log(data[0])
  const {id, title, descr, cover,average_score, media_type, release_date} = data[0];
  const full_date = moment(release_date).format("YYYY-MM-DD");
  ...
  formElement.innerHTML = `
    ...
    <form id="form">
      ...
      <fieldset>
        ...
        <input placeholder="" type="tel" tabindex="3" name="votes_update" value = "${average_score}" id="calificacion">
      </fieldset>
      <fieldset>
        <label> Fecha de estreno (formato yyyy-mm-dd)* </label>
        <input placeholder="" type="tel" tabindex="4" name="release_update" value = "${full_date}" id="estreno">
      </fieldset>
      <fieldset>
